...
function firstOffer(node: Record<string, unknown>): Record<string, unknown> | null {
  ...
  if (offers && typeof offers === 'object') {
    ...
    if (Array.isArray(record.offers)) return (record.offers[0] as Record<string, unknown>) ?? null;
    return record;
  }
  return null;
}
...
export function deriveCategories(...inputs: (string | null | undefined)[]): string[] {
  ...
  for (const [tag, words] of Object.entries(dictionary)) {
    if (words.some((word) => normalized.includes(word))) tags.add(tag);
  }
  ...
  for (const segment of haystack.split(/[\s/>|,.-]+/)) {
    ...
    if (clean.length >= 4 && clean.length <= 20 && !stopwords.has(clean)) tags.add(clean);
  }
  return [...tags];
}

/** Extrai dados do produto a partir do HTML da página. */
export function extractProduct(url: string, html: string): Product {
